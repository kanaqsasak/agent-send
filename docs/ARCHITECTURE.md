...
The daemon does not yet own a LAN control listener or a socket-backed `PeerTransport`: connecting that listener to platform firewall prompts, sleep/wake, interface changes, and the persisted pairing-secret exchange requires OS integration that has not been validated on macOS, Windows, and Linux. The local API remains loopback-only. mDNS is real socket integration, but Wi-Fi client isolation and firewall behavior remain deployment tests rather than claimed cross-platform guarantees.
